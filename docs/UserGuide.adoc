...
[NOTE]
If the .jar file is moved to another location, you should move the data folder as well. If this is not done, the app
will not be able to locate the data and will start with the default settings. Refer to <<FAQ>>

[WARNING]
The data is stored as an editable xml file. If the user manually edits this file, _NUSCouples_ may not be able to read
it properly. _NUSCouples_ takes no responsibility for data loss/ data corruption due to editing these files.
Refer to <<FAQ>>
...
folder in the original directory was moved to the new directory. +
