...
Firstly, it uses Outh 2.0 endpoints to allow users to share specific data with the application while keeping their usernames, passwords, and other information private. For example, an application can use OAuth 2.0 to obtain permission from users to store files in their Google Drives which sync to the calendar.
The only downside to this implementation is that the user needs to connect to the internet because NUSCouples app needs to open the system browser and supply a local redirect URI to handle responses from Google's authorization server.
...
image::CalendarSequence.png[width="850"]
...
** Pros: I will learnt more even i failed at the end and Google API is more versatile
