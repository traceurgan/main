...
== Introduction
Welcome to our developer guide for NUSCouples!!
Over the time in NUS, our team has witness many breakups in NUS due to various difficulties to balance school work and their relationship. Which is heartbreaking!
Thus, we have developed this desktop application so that it can help them store their memories and keep each other updated!
NUSCouples App is a command-line application that can provide convenient ways to manipulate the data. Moreover, we try to keep every command as simple as it can to accommodate users that are not familiar with CLI interface.
This developer guide contains information that can help developers to get started as a NUSCouples contributor even if you are an experienced developer.
If you face any uncertainty in the instructions, feel free to email to marlene.koh@u.nus.edu, our friendly and pretty assistant will help you to sort out the issues for you accordingly.

*This developer guide consist of following sections:*
...
It supports viewing/add/editing/deleting capability that modifies the state of the NUSCouples App.
...
This implementation requires the user to connect to the internet because NUSCouples app needs to open the system browser and supply a local redirect URI to handle responses from Google's authorization server.
...
Users are able to add a shortened https://nusmods.com/[NUSMods] timetable URL to their existing partner in NUSCouples. +
...
The journal feature is implemented by a `Journal`, which resides inside `ModelManager`. It allows the user to create and save journal entries.
...
Suppose that the user has just launched the application for the first time. The `Journal` will be empty at the beginning.

The following sequence diagram shows how the newJournal operation works:

image::JournalWindowUIComponentSequenceDiagram.png[width="800"]
...
|Daniel
|Location Finder: To allow the user to find the venue of a particular lesson of interest from their own timetable using
a Google Maps search result
|Communication with Google API: Can communicate with different certain external APIs from Google such as Google Maps API
...
1. User are required to generate and download their credential from google API credentials: https://console.developers.google.com/apis/dashboard?
2. User are required to import their credential into the project resources directory.
3. NUSCouples add the credential of the User to authenticate with Google.
+
Use case ends.
...
discrete]
=== Use case: View Calendar User of User

*MSS*

1.User inside browser panel enter's the command to view calendar.
2.NUSCouple redirects to google Calendar to retrieve data.
3.NUSCouple Display the UI accordingly.
...
** 1a1. NUSCouples redirects to google calendar account to authenticate using the user credential.
** 1a2. User confirms change.
** 1a3. NUSCouples list the calendar in browser panel.
+
Use case resumes at step 2.
